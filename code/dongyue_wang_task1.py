...
head = rawrdd.first()
small1rdd = rawrdd.filter(lambda x: x != head)

## Create a basket for each user/business
user_basket = small1rdd.map(lambda x: (x[0], [x[1]])).sortByKey().reduceByKey(lambda a, b: a + b)

bus_basket = small1rdd.map(lambda x: (x[1], [x[0]])).sortByKey().reduceByKey(lambda a, b: a + b)

if case == 1:
    chunks_rdd = user_basket.map(lambda x: x[1])
else:
    chunks_rdd = bus_basket.map(lambda x: x[1])
